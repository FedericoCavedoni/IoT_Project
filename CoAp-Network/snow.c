...
#define SENSOR_TYPE "snow_machine"
...
extern coap_resource_t res_snow;

PROCESS(coap_snow, "CoAP Snow Machine actuator");
AUTOSTART_PROCESSES(&coap_snow);
...
PROCESS_THREAD(coap_snow, ev, data){
  PROCESS_BEGIN();
  ...
  LOG_INFO("Snow Machine Actuator started\n");
  coap_activate_resource(&res_snow, "snow_machine");
...
// try to connect to the border router
  ...
  while(1) {
    ...
    if(ev == button_hal_press_event) {
		// changing the status of the snow machine
