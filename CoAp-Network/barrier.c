...
extern coap_resource_t res_barrier;

PROCESS(coap_barrier, "Coap Barrier Actuator");
AUTOSTART_PROCESSES(&coap_barrier);
...
PROCESS_THREAD(coap_barrier, ev, data){
  PROCESS_BEGIN();
  ...
  LOG_INFO("Barrier Actuator started\n");
  coap_activate_resource(&res_barrier, "barrier");
  ...
  while(1) {
    ...
    if(ev == button_hal_press_event) {
        // changing the status of the barrier
